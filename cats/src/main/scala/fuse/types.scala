...
object TableId {
  def fromTableName(table: TableName): Either[TableIdentifierError, TableIdentifier] =
    table.render.split('.').toList match {
      case tname :: Nil =>
        TableIdentifier(tname, None).asRight
      case db :: tname :: Nil =>
        TableIdentifier(tname, Some(db)).asRight
      case _ =>
        TableIdentifierError.invalidFormat(table).asLeft
    }
}
