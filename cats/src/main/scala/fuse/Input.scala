...
object Input {
  ...
  def load(session: SparkSession, input: Input): Either[InputLoadError, DataFrame] =
    loadWithMetadata(session, input).map(_.data)

  def loadWithMetadata(session: SparkSession, input: Input): Either[InputLoadError, InputData] =
    input match {
      case Input.Table(inputTable) =>
        ...
        InputData(InputMetadata.table(loadTable(session, inputTable)), df).asRight
      case Input.ParquetFile(baseDir, props) =>
        try {
          val df = ParquetInputProperties.reader(session.read, props).parquet(baseDir.value)
          InputData(InputMetadata.parquetFile(baseDir, df.schema, props), df).asRight
        } catch {
          case e: AnalysisException =>
            InputLoadError.file(e).asLeft
        }
      case Input.CsvFile(baseDir, props) =>
        try {
          val df = CsvInputProperties.reader(session.read, props).csv(baseDir.value)
          InputData(InputMetadata.csvFile(baseDir, df.schema, props), df).asRight
        } catch {
          case e: AnalysisException =>
            InputLoadError.file(e).asLeft
        }
  ...
  def loadData[A](session: SparkSession, input: Input)(implicit E: DataEncoder[A]): Either[InputLoadDataError, Data[A]] =
    loadDataWithSchema(session, input)(_ => E)

  def loadDataWithSchema[A](session: SparkSession, input: Input)(E: StructType => DataEncoder[A]): Either[InputLoadDataError, Data[A]] =
    for {
