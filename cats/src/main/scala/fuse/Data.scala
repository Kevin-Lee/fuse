  ...
  def joinLeftOuter[B](right: Data[B], on1: String, onRest: String*): Data[(A, Option[B])] = {
    ...
    val enc = new DataEncoder[(A, Option[B])] {
      ...
      override def fromRow: RowParser[(A, Option[B])] = {
        import RowParser._
        val rowA: RowParser[A] = getAs[Row].flatMapFail(E.fromRow.apply)
        val rowOptionB: RowParser[Option[B]] =
          getAsOption[Row].flatMapFail {
            case None =>
              none[B].asRight[RowParserError]
            case Some(b) =>
              right.E.fromRow(b).map(Option(_))
          }
        (
          rowA
        , rowOptionB
...
object Data {
  ...
  type Validated[A] = Either[SchemaErrors, A]
  ...
  def fromDataset[A](rows: Dataset[Row])(implicit E: DataEncoder[A]): Validated[Data[A]] =
    DataEncoder.validateSchema(E, rows.schema).map(_ => new Data(rows, E))
...
object DataEncoder {
   ...
   */
  def validateSchema[A](enc: DataEncoder[A], schema: StructType): Either[SchemaErrors, Unit] =
    enc.schema.fields.toList.foldMap(sf => (for {
      o <- schema.fields.find(_.name === sf.name).toRight(SchemaError.missingField(sf.name))
      _ <-
        if(Eq.fromUniversalEquals.eqv(o.dataType, sf.dataType))
          ().asRight
        else
          SchemaError.invalidType(sf.name, sf.dataType, o.dataType).asLeft
    } yield ()).toValidatedNel).toEither.leftMap(SchemaErrors(_))
}

case class SchemaErrors(errors: NonEmptyList[SchemaError]) {
  def render: String =
    errors.toList.map(_.render).mkString("\n")
...
object SchemaErrors {
  def create(error: SchemaError, others: SchemaError*): SchemaErrors =
    SchemaErrors(NonEmptyList(error, others.toList))
}
